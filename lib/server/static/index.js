...
$('#search').on('keyup', function(e) {
	...
	$('.schema-list a').each(function(index, aTag) {
		reg.test($(aTag).text().toLowerCase())? $(aTag).parent('.schema').removeClass('hide'): $(aTag).parent('.schema').addClass('hide')
	})
})
...
$('#schema-content').on('click', '.mock-wrap a', function(e) {
	...
	if($(e.target).attr('data-from') === 'schemaData' || $(e.target).text() === '随机') {
		$('#subm').hide()
		$('#mock-delete').hide()
	} else {
		$('#subm').show()
...
$('#schema-content').on('keyup', '#path', function(e) {
	if (e.keyCode === 13) {
		e.target.value = '/' + e.target.value
		$(e.target).blur()
	}
